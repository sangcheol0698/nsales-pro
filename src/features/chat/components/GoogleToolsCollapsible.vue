  ...
  <div class="space-y-3">
    ...
    <div v-if="gmailResult" class="group">
      <Collapsible v-model:open="isGmailOpen">
        <CollapsibleTrigger class="w-full">
          <div
            class="flex items-center justify-between p-3 rounded-lg border border-emerald-200/50 bg-gradient-to-r from-emerald-50/50 to-blue-50/50 hover:from-emerald-100/70 hover:to-blue-100/70 transition-all duration-300 hover:shadow-md hover:shadow-emerald-500/10 group-hover:border-emerald-300/70">
            <div class="flex items-center gap-3">
              <div class="relative">
                <Mail class="h-5 w-5 text-emerald-600" />
                <div
                  class="absolute -inset-1 bg-emerald-500/20 rounded-full animate-pulse opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              </div>
              <div class="text-left">
                <h3
                  class="font-semibold text-sm text-emerald-800 bg-gradient-to-r from-emerald-700 to-blue-700 bg-clip-text text-transparent">
                  {{ gmailResult.title }}
                </h3>
    ...
    <div v-if="calendarResult" class="group">
      <Collapsible v-model:open="isCalendarOpen">
        <CollapsibleTrigger class="w-full">
          <div
            class="flex items-center justify-between p-3 rounded-lg border border-blue-200/50 bg-gradient-to-r from-blue-50/50 to-purple-50/50 hover:from-blue-100/70 hover:to-purple-100/70 transition-all duration-300 hover:shadow-md hover:shadow-blue-500/10 group-hover:border-blue-300/70">
            <div class="flex items-center gap-3">
              <div class="relative">
                <Calendar class="h-5 w-5 text-blue-600" />
                <div
                  class="absolute -inset-1 bg-blue-500/20 rounded-full animate-pulse opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              </div>
              <div class="text-left">
                <h3
                  class="font-semibold text-sm text-blue-800 bg-gradient-to-r from-blue-700 to-purple-700 bg-clip-text text-transparent">
...
import { Calendar, ChevronDown, Mail } from 'lucide-vue-next';
