  ...
  >
    <form @submit.prevent="handleSubmit(onSubmit)" class="space-y-6">
      ...
      <div class="flex justify-end gap-2 pt-4 border-t">
        ...
        <Button type="submit" :disabled="loading" @click="handleButtonClick">
...
const handleOrgSelected = (node: { departmentId?: number }) => {
  if (node?.departmentId && currentFieldRef.value) {
    // FormField의 onChange를 사용해서 값 업데이트
...
const onSubmit = (values: any) => {
  ...
  console.log('변환된 프로젝트 데이터:', projectData);

  const project = new ProjectCreate(projectData);
  console.log('생성된 프로젝트 객체:', project);
...
const handleButtonClick = (event: Event) => {
  console.log('프로젝트 생성 버튼 클릭됨:', event);
  // event.preventDefault() 제거 - 폼 제출이 되도록 함
};
