...
interface PartnerFormEmits {
  submit: [partner: PartnerCreate];
  cancel: [];
}

const props = withDefaults(defineProps<PartnerFormProps>(), {
  loading: false,
});

const emit = defineEmits<PartnerFormEmits>();
...
const onSubmit = (values: any) => {
  console.log('협력사 폼 제출 시작:', values);
  ...
  console.log('변환된 협력사 데이터:', partnerData);

  const partner = new PartnerCreate(partnerData);
  console.log('생성된 협력사 객체:', partner);
